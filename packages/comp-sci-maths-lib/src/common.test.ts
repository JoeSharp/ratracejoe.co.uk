...
describe("Common", () => {
  ...
  describe("validateArrayIndices", () => {
    it("Throws error for index too high", () => {
      expect(() => validateArrayIndices([1, 2, 3], 5)).toThrow();
    });
    it("Throws error for index too high amongst valid", () => {
      expect(() => validateArrayIndices([1, 2, 3], 0, 5, 1)).toThrow();
    });

    it("Throws error for negative index", () => {
      expect(() => validateArrayIndices([1, 2, 3], -1)).toThrow();
    });

    it("Throws error for negative index amongst valid ones", () => {
      expect(() => validateArrayIndices([1, 2, 3], -1, 0, 1)).toThrow();
    });

    it("Doesn`t throw error for valid index", () => {
      expect(() => validateArrayIndices([1, 2, 3], 2)).not.toThrow();
    });

    it("Doesn`t throw error for valid indices", () => {
      expect(() => validateArrayIndices([1, 2, 3], 0, 1, 2)).not.toThrow();
    });
  });
