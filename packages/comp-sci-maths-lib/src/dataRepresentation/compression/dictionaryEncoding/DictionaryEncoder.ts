...
class DictionaryEncoder {
  ...
  withText(theText: string): this {
    ...
    const theMatch = theText.match(FIND_BITS_REGEX);
    if (theMatch === null) return this;
    theMatch.forEach((word) => {
  ...
  encode(): string {
    return [
      DICTIONARY_HEADER,
      ...Object.entries(this.wordsByNumber).map(
        ([id, word]) => `${id}=${word}`,
      ),
      ENCODED_HEADER,
      ...this.encoded,
   ...
   */
  rebuild(
    transformWord: (id: number, word: string) => string = (id, word) => word,
  ): string {
    return this.encoded
