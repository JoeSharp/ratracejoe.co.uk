...
class PatternSearchTree<T> {
  ...
  equalityCheck: EqualityCheck<Optional<T>>;
  ...
  constructor(
    ...
    this.equalityCheck = (a, b) => !!a && !!b && equalityCheck(a, b);
    this.root = new Tree({
      value: undefined,
      sequenceThusFar: [],
      count: 0,
  ...
  count(...values: T[]): number {
    let item: Optional<Tree<TreeCount<T>>> = this.root;

    if (values.length >= this.maxLength)
      throw new Error(
        `This search tree is only configured for patterns up to ${this.maxLength} long`,
      );

    for (const value of values) {
      item = item
        .getChildren()
        .find((child) => this.equalityCheck(value, child.getValue().value));
  ...
  record(...values: T[]): this {
    ...
    while (values.length > 0) {
      ...
      for (const [index, value] of values.entries()) {
        ...
        let childItem: Optional<Tree<TreeCount<T>>> = item
          .getChildren()
          .find((child) => this.equalityCheck(value, child.getValue().value));
        if (!!childItem) {
