...
describe("MasterMind - Row", () => {
  test("Exact Match", () => {
    ...
    const row = new Row(ROW_SIZE).withValues(1, 5, 3, 3);
  ...
  test("Lots of similair numbers", () => {
    const target = new Row(ROW_SIZE).withValues(0, 2, 0, 0);
    const row = new Row(ROW_SIZE).withValues(0, 0, 2, 0);
  ...
  test("Couple correct", () => {
    const target = new Row(ROW_SIZE).withValues(1, 5, 3, 3);
    const row = new Row(ROW_SIZE).withValues(1, 5, 7, 2);
  ...
  test("One correct, two in wrong place", () => {
    const target = new Row(ROW_SIZE).withValues(1, 5, 3, 3);
    const row = new Row(ROW_SIZE).withValues(1, 3, 5, 2);
  ...
  test("Mismatching 'target' row", () => {
    ...
    expect(() => row.evaluate(target)).toThrow();
  });

  test("Invalid Index on Build", () => {
    const row = new Row(ROW_SIZE);
    expect(() => row.withValue(row.values.length + 1, 1)).toThrow();
    expect(() => row.withValue(-9, 1)).toThrow();
  });
});
