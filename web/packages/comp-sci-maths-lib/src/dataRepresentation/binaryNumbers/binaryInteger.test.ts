import { BinaryNumber, DEFAULT_BIT_VALUE } from "./types";
...
describe("Binary Integers", () => {
  ...
  test("toggleBitInBinary", () => {
    ...
    expect(() => toggleBitInBinary(value, -1)).toThrow();
    expect(() => toggleBitInBinary(value, 4)).toThrow();
    expect(() => toggleBitInBinary(value, 67)).toThrow();
  });
  ...
  describe("shiftRight", () => {
    SHIFT_RIGHT_TEST_CASES.forEach(
      ({ binary, result, gapFill = DEFAULT_BIT_VALUE, flag }) =>
