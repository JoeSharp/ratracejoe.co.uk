...
export type GraphAction<T = string> =
  | AddVertexAction<T>
  | GraphRemoveVertex
  | GraphRemoveEdge
 ...
 */
export const getVertex = <T = string>(
  { vertices }: Graph<T>,
  asString: string,
): Optional<string> => vertices.find((v) => v === asString);
 ...
 */
export const getVertexValue = <T = string>(
  { vertexValues }: Graph<T>,
  vertex: string,
): Optional<T> =>
  vertexValues !== undefined ? vertexValues[vertex] : undefined;
 ...
 */
export const getEdge = <T = string>(
  { edges }: Graph<T>,
  from: string,
  to: string,
): Optional<Edge> => edges.find((l) => l.from === from && l.to === to);

/**
 * Access edges coming into a specific vertex
 * @param vertex The from vertex
 */
export const getIncoming = <T = string>(
  { edges }: Graph<T>,
  toKey: string,
): Edge[] => edges.filter((l) => l.to === toKey);
 ...
 * @returns The edges
 */
export const getOutgoing = <T = string>(
  { edges }: Graph<T>,
  fromKey: string,
): Edge[] => edges.filter((l) => l.from === fromKey);
 ...
 */
export const getEdgeWeight = <T = string>(
  state: Graph<T>,
  from: string,
  to: string,
): number => {
 ...
 */
export const addVertex = <T = string>(
  state: Graph<T>,
  vertex: string,
  value: Optional<T> = undefined,
): Graph<T> => ({
  ...state,
  vertices: [...state.vertices.filter((v) => v !== vertex), vertex],
  vertexValues: value
    ? {
        ...state.vertexValues,
        [vertex]: value,
...
export const disconnectVertex = <T = string>(
  state: Graph<T>,
  vertex: string,
): Graph<T> => ({
  ...state,
 ...
 * @returns new graph state after changes
 */
export const removeVertex = <T = string>(
  state: Graph<T>,
  vertex: string,
): Graph<T> => ({
  ...state,
 ...
 * @returns new graph state after changes
 */
export const removeEdge = <T = string>(
  state: Graph<T>,
  from: string,
  to: string,
): Graph<T> => ({
...
export const addUnidirectionalEdge = <T = string>(
  ...
  vertices: [
    ...state.vertices,
    ...[from, to].filter(
      (vertex) => state.vertices.find((v) => v === vertex) === undefined,
    ),
  ],
  edges: [
    ...state.edges.filter((l) => !(l.from === from && l.to === to)), // filter any existing Edge in this direction
    { from, to, weight },
  ],
});
 ...
 * @returns new graph state after changes
 */
export const reduce = <T = string>(
  state: Graph<T>,
  action: GraphAction<T>,
): Graph<T> => {
  switch (action.type) {
    case "addVertex":
      return addVertex(state, action.vertex, action.value);
    case "removeVertex":
      return removeVertex(state, action.vertex);
    case "removeEdge":
      return removeEdge(state, action.from, action.to);
    case "addUnidirectionalEdge":
      return addUnidirectionalEdge(
        state,
        action.from,
        action.to,
        action.weight,
    ...
    case "addBidirectionalEdge":
      return addBidirectionalEdge(state, action.from, action.to, action.weight);
    case "reset":
      return createInitialState();
    case "disconnectVertex":
      return disconnectVertex(state, action.vertex);
    case "replace":
