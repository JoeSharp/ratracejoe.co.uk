...
export const getInitialPriorityQueueState = <T>(
  capacity: number = DEFAULT_CAPACITY,
): PriorityQueueState<T> =>
  getInitialLinkedListState<PrioritisedItem<T>>(capacity);

export type PriorityQueueAction<T> =
  | EnqueuePrioritisedAction<T>
  | DequeuePrioritised
  | PeekPrioritised
  | ResetAction;

export const priorityQueueEnqueue = <T>(
  ...
  if (insertIndex === undefined) {
    newState = linkedListReducer(state, {
      type: "append",
      value: newPrioritisedItem,
    });
  } else {
    newState = linkedListReducer(state, {
      type: "insert",
...
export const priorityQueuePeek = <T>(
  state: PriorityQueueState<T>,
): PriorityQueueState<T> => ({
  ...state,
 ...
 */
export const priorityQueueReducer = <T>(
  state: PriorityQueueState<T>,
  action: PriorityQueueAction<T>,
): PriorityQueueState<T> => {
  switch (action.type) {
    case "enqueue":
      return priorityQueueEnqueue(state, action.value, action.priority);
    case "dequeue":
      return priorityQueueDequeue(state);
    case "peek":
      return priorityQueuePeek(state);
    case "reset":
