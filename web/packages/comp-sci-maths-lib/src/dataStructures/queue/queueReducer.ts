...
export type QueueState<T> = LinearStructureState<T> & {
  front: number;
  rear: number;
};
...
export const getInitialQueueState = <T>(
...
export const isQueueEmpty = ({ front }: QueueState<any>): boolean =>
  front === FRONT_IF_EMPTY;
export const isQueueFull = ({ front, rear }: QueueState<any>): boolean =>
  front === rear;
...
export const queueReducer = <T>(
  state: QueueState<T>,
  action: LinearStructureAction<T>,
): QueueState<T> => {
  switch (action.type) {
    case "push":
      return queuePush(state, action.value);
    case "pop":
      return queuePop(state);
    case "peek":
      return queuePeek(state);
    case "reset":
      return getInitialQueueState();
  }
};
