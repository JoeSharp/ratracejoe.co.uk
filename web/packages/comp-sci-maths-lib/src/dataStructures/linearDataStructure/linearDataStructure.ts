...
export type LinearStructureAction<T> =
  | LinearStructureActionPush<T>
...
export const getInitialLinearStructureState = <T>(
  capacity: number = DEFAULT_CAPACITY,
): LinearStructureState<T> => ({
...
export const linearStructureError = <STATE extends LinearStructureState<any>>(
  state: STATE,
  lastMessage: LinearDataStructureMessages,
): STATE => ({
  ...state,
...
export type LinearDataStructureReducer<
  T,
  STATE extends LinearStructureState<T>,
> = (state: STATE, action: LinearStructureAction<T>) => STATE;
