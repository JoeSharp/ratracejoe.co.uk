...
export interface LinkedListItem<T> {
  value: T;
  nextPtr: Optional<number>;
}

export interface LinkedListState<T> extends LinearStructureState<
  LinkedListItem<T>
> {
  freeNodes: StackState<number>;
  start: Optional<number>;
  nextFree: Optional<number>;
}

export const getInitialLinkedListState = <T>(
...
export type LinkedListAction<T> =
  | LinkedListAppendAction<T>
  | LinkedListRemoveMatchAction<T>
  | LinkedListRemoveAction
  | LinkedListInsertAction<T>
  | LinkedListResetAction;

export const isListEmpty = ({ start }: LinkedListState<any>): boolean =>
  start === undefined;
export const isListFull = ({
  capacity,
  nextFree,
  freeNodes,
}: LinkedListState<any>): boolean =>
  isStackEmpty(freeNodes) && !!nextFree && nextFree >= capacity;
 ...
 */
export const linkedListTraverse = <T>(
  state: LinkedListState<T>,
  stop: (
    item: Optional<LinkedListItem<T>>,
    logicalIndex: number,
  ...
  while (
    currPhysicalIndex !== undefined &&
    !stop(
      state.contents[currPhysicalIndex],
      currLogicalIndex,
      currPhysicalIndex,
    )
  ) {
 ...
 * @returns
 */
export const linkedListGet = <T>(
  state: LinkedListState<T>,
  logicalIndex: number,
): Optional<LinkedListItem<T>> => {
  let physicalIndex = undefined;
  ...
  return physicalIndex !== undefined
    ? state.contents[physicalIndex]
    : undefined;
};
...
export const linkedListGetStoreIndex = <T>(
  state: LinkedListState<T>,
): {
  newPhysicalIndex: Optional<number>;
  nextFree: Optional<number>;
  freeNodes: StackState<number>;
} => {
  let newPhysicalIndex = undefined;
  let freeNodes = state.freeNodes;
  let nextFree = state.nextFree;
  if (isStackEmpty(state.freeNodes)) {
    if (nextFree === undefined) throw new Error("Ran out of free");
...
export const linkedListAppend = <T>(
  ...
  const lastItem: Optional<LinkedListItem<T>> =
    indexes.length > 0
      ? state.contents[indexes[indexes.length - 1]]
      : undefined;
  ...
  if (newPhysicalIndex === undefined || newPhysicalIndex === undefined) {
...
export const linkedListRemoveLogicalIndex = <T>(
  state: LinkedListState<T>,
  logicalIndex: number,
  ...
  const contents = [...state.contents];
  let start: number | undefined = state.start;
  let freeNodes = state.freeNodes;
...
export const linkedListRemoveMatch = <T>(
  state: LinkedListState<T>,
  match: MatchFunction<T>,
): LinkedListState<T> => {
  ...
  const physicalIndexes = linkedListTraverse(
    state,
    (_, currLogicalIndex, currPhysicalIndex) => {
      if (match(state.contents[currPhysicalIndex as number]?.value!)) {
  ...
  if (logicalIndex === undefined || physicalIndex === undefined)
...
export const linkedListRemove = <T>(
  state: LinkedListState<T>,
  logicalIndex: number,
): LinkedListState<T> => {
  if (isListEmpty(state))
  ...
  const physicalIndexes = linkedListTraverse(
    state,
    (_, currLogicalIndex) => currLogicalIndex > logicalIndex,
  );
  return linkedListRemoveLogicalIndex(state, logicalIndex, physicalIndexes);
};

export const linkedListInsert = <T>(
  state: LinkedListState<T>,
  logicalIndex: number,
  value: T,
): LinkedListState<T> => {
  // Make copies of those state items likely to change
  let start: Optional<number> = state.start;
  const contents = [...state.contents];
  const newItem: LinkedListItem<T> = {
    value,
    nextPtr: undefined,
...
export const linkedListReducer = <T>(
  state: LinkedListState<T>,
  action: LinkedListAction<T>,
): LinkedListState<T> => {
  switch (action.type) {
    case "append":
      return linkedListAppend(state, action.value);
    case "remove":
      return linkedListRemove(state, action.logicalIndex);
    case "removeMatch":
      return linkedListRemoveMatch(state, action.match);
    case "insert":
      return linkedListInsert(state, action.logicalIndex, action.value);
    case "reset":
      return getInitialLinkedListState();
  }
};
