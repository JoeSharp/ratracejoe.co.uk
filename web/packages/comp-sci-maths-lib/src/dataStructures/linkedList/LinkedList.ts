  ...
  *[Symbol.iterator]() {
  ...
  insert(index: number, item: T) {
    ...
    if (index === 0) {
      ...
      let tIndex: number = 1;
      let currentItem: Optional<LinkedItem<T>> = this.startItem;
      while (!!currentItem) {
        if (tIndex === index) {
  ...
  append(item: T): this {
    ...
    if (!!this.startItem) {
      let currentItem: LinkedItem<T> | undefined = this.startItem;
  ...
  findMatch(matchFunction: MatchFunction<T>): number {
    let index: number = 0;
    let cItem: Optional<LinkedItem<T>> = this.startItem;
    while (!!cItem) {
