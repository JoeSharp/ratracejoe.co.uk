...
export type StackState<T> = LinearStructureState<T> & {
  stackPointer: number;
};

export const getInitialStackState = <T>(
...
export const isStackEmpty = ({ stackPointer }: StackState<any>): boolean =>
  stackPointer === 0;
export const isStackFull = ({
  stackPointer,
  capacity,
}: StackState<any>): boolean => stackPointer === capacity;
...
export const stackReducer = <T>(
  ...
  switch (action.type) {
    case "push":
      return stackPush(state, action.value);
    case "pop":
      return stackPop(state);
    case "peek":
      return stackPeek(state);
    case "reset":
      return getInitialStackState();
  }
};
