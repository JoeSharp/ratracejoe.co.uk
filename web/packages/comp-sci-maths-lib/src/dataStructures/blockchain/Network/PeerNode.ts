...
class PeerNode {
  ...
  popMessage(): Message {
    ...
    if (!otherPublicKey) throw new Error("No other public key");
    if (!msg.content) throw new Error("Nothing to decrypt");
    if (!msg.signature) throw new Error("No signature");
  ...
  sendMessage(destinationId: string, content: string) {
    ...
    if (!otherPublicKey) throw new Error("No public key");
